...
class EDDSATest : public ::testing::Test {
  ...
  void TestEd25519_Sign(const std::string secret, const std::string p,
    ...
    SECOidTag curveOidTag = SEC_OID_CURVE25519; /* default */
    oidData = SECOID_FindOIDByTag(curveOidTag);
    ...
    EC_DecodeParams(ecparams, &tmpECParams);
    tmpECParams->name = ECCurve25519;
    tmpECParams->fieldID.type = ec_field_plain;
    ...
    key.ecParams = *tmpECParams;

    SECStatus rv = EDDSA_SignDigest(&key, signature_item.get(), &msg_value);
    ASSERT_EQ(SECSuccess, rv);

    ECPublicKey public_key;
    public_key.publicValue = public_value;
    public_key.ecParams = *tmpECParams;

    rv = EDDSA_VerifyDigest(&public_key, signature_item.get(), &msg_value);
    ASSERT_EQ(SECSuccess, rv);
    ...
    ECPublicKey new_public_key;
    SECStatus rv1 = SECFailure;
    rv1 = ED_NewKey(tmpECParams, &newKey);
    ASSERT_EQ(SECSuccess, rv);
    if(newKey == NULL){
        printf("Null key.\n");
    }
    ...
    new_public_key.publicValue = newKey->publicValue;
    //Hacl_Ed25519_secret_to_public(new_public_key.publicValue.data, newKey->privateValue.data);
    new_public_key.ecParams = *tmpECParams;

    rv1 = EDDSA_SignDigest(newKey, signature_item.get(), &msg_value);
    ASSERT_EQ(SECSuccess, rv1);

    rv1 = EDDSA_VerifyDigest(&new_public_key, signature_item.get(), &msg_value);
    ASSERT_EQ(SECSuccess, rv1);


 /*   ScopedSECItem signature_item1(SECITEM_AllocItem(nullptr, nullptr, 64U));

    uint8_t *privKeyBytes = NULL, *pubKeyBytes = NULL;
    if (((privKeyBytes = PORT_Alloc(32)) == NULL) || ((pubKeyBytes = PORT_Alloc(64)) == NULL))
        printf("Key Allocation failed.\n");

    RNG_GenerateGlobalRandomBytes(privKeyBytes, 32));
    std::vector<uint8_t> p_bytes, pub_bytes;
    Hacl_Ed25519_secret_to_public(pubKeyBytes, privKeyBytes);

    for(uint8_t i = 0; i < 32; i++){
        p_bytes.push_back(privKeyBytes[i]);
        std::cout << p_bytes[i];
    }
    ASSERT_EQ(p_bytes.size(), 32U);

    for(uint8_t i = 0; i < 64; i++){
        pub_bytes.push_back(pubKeyBytes[i]);
        std::cout << pub_bytes[i];
    }
    ASSERT_EQ(p_bytes.size(), 64U);

    SECItem p_value = {siBuffer, p_bytes.data(),
                            static_cast<unsigned int>(p_bytes.size())};
    SECItem pub_value = {siBuffer, pub_bytes.data(),
                            static_cast<unsigned int>(pub_bytes.size())};

    ECPrivateKey newKey;
    newKey.privateValue = p_value;
    key.ecParams = *tmpECParams;

    SECStatus rv1 = EDDSA_SignDigest(&key, signature_item1.get(), &msg_value);
    ASSERT_EQ(SECSuccess, rv1);

    ECPublicKey new_public_key;
    new_public_key.publicValue = pub_value;
    new_public_key.ecParams = *tmpECParams;

    rv1 = EDDSA_VerifyDigest(&new_public_key, signature_item1.get(), &msg_value);
    ASSERT_EQ(SECSuccess, rv1);
*/
...
/*    ECPrivateKey *newKey = NULL;
      ECPublicKey new_public_key;
      SECStatus rv1 = SECFailure;
      SECStatus rv1 = ED_NewKey(tmpECParams, &newKey);
      ASSERT_EQ(SECSuccess, rv);
